...
3. Update `compatibility.json` with versions, class hashes, and addresses.
4. Run `pnpm install`, `pnpm check`, and `pnpm shadow:doctor`.
5. Run the credentialed Sepolia lifecycle from a fresh nonce.
6. Confirm the outer sender, event, registry, class hash, and effect assertions.
7. Only then update the default `.env.example` values.
